...
class ResizablePage(BasePage):
    ...
    def change_size_resizable(self):
        self.remove_element(self.locators.FOOTER)
...
class DroppablePage(BasePage):
    ...
    def drop_accept(self):
        ...
        drop_acceptable_div = self.element_is_visible(self.locators.DROP_HERE_ACCEPT)
        self.action_drag_and_drop_to_element(not_acceptable_div, drop_acceptable_div)
        text_not_acceptable = drop_acceptable_div.text
        self.action_drag_and_drop_to_element(acceptable_div, drop_acceptable_div)
        text_acceptable = drop_acceptable_div.text
        return text_not_acceptable, text_acceptable

    def drop_prevent_propgation(self):
...
class DraggablePage(BasePage):
    ...
    def axis_restricted_x(self):
        ...
        pos_x = self.get_before_and_after_positions(only_x)
        top_x_before = self.get_top_position(pos_x[0])
        top_x_after = self.get_top_position(pos_x[1])
        left_x_before = self.get_left_position(pos_x[0])
        left_x_after = self.get_left_position(pos_x[1])
    ...
    def axis_restricted_y(self):
        ...
        pos_y = self.get_before_and_after_positions(only_y)
        top_y_before = self.get_top_position(pos_y[0])
        top_y_after = self.get_top_position(pos_y[1])
        left_y_before = self.get_left_position(pos_y[0])
        left_y_after = self.get_left_position(pos_y[1])
